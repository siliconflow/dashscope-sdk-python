...
开发一个转发到 OpenAI 标准服务器的 proxy，我们成为 DeepSeek-DashScope Proxy。
## 兼容模式
将
create_mock_server，
create_app，
...
以下是合作方的对接文档
这是一个基于 `FastAPI` 和 `openai` Python SDK 构建的 Proxy 实现。

它严格遵循你提供的 DashScope 接入规范，实现了：

1. **输入兼容**：自动转换 `prompt/history`（旧版）到 `messages`（新版）。
2. **输出统一**：统一封装为 DashScope 的 JSON 结构。
3. **流式透传**：支持 SSE，并处理了 DeepSeek 特有的 `reasoning_content` 和全量 `usage` 回传。
4. **接口修复**：修复了 `create_mock_server` 的签名问题。
...
# 模型映射表: DashScope请求名 -> SiliconFlow/DeepSeek 实际名
MODEL_MAPPING = {
...
确保在运行前设置：
```bash
export SILICON_FLOW_API_KEY="你的key"

```


3. **关于 `create_mock_server` 的修改:**
我在 `create_mock_server` 中添加了 `*args` 和 `**kwargs`。这解决了你遇到的 `TypeError`，因为某些测试框架在调用此钩子时会隐式传递配置对象或参数，现在它们会被安全地忽略，同时正确返回 `app` 实例。
4. **DeepSeek 特性处理:**
* **Reasoning Content**: 代码中通过 `getattr(delta, "reasoning_content", "")` 尝试从 SiliconFlow 的响应中提取思考过程（DeepSeek R1）。
* **Usage**: 实现了流式响应中每一包都携带 `usage` 数据的逻辑（依赖 OpenAI SDK 的 `stream_options={"include_usage": True}`）。


以下为 tests/mock_server.py
...
from tests.constants import (TEST_DISABLE_DATA_INSPECTION_REQUEST_ID,
                             TEST_ENABLE_DATA_INSPECTION_REQUEST_ID,
                             TEST_JOB_ID)
from tests.handle_deployment_request import (create_deployment_handler,
                                             delete_deployment_handler,
                                             events_deployment_handler,
                                             get_deployment_handler,
                                             list_deployment_handler)
from tests.handle_fine_tune_request import (cancel_fine_tune_handler,
                                            create_fine_tune_handler,
                                            delete_fine_tune_handler,
                                            events_fine_tune_handler,
                                            get_fine_tune_handler,
                                            list_fine_tune_handler)
from tests.mock_sse import sse_response
from tests.websocket_mock_server_task_handler import WebSocketTaskProcessor


def validate_data_inspection_parameter(request: aiohttp.request):
    if ('request_id' in request.headers and request.headers['request_id']
            == TEST_ENABLE_DATA_INSPECTION_REQUEST_ID):
...
async def response_403(request: aiohttp.request):
    ...
    async for msg in ws:
        if msg.type == aiohttp.WSMsgType.TEXT:
            ...
            if req['header']['action'] == ActionType.START:
                ...
                print('receive first payload: %s' % req['payload'])
                wsc = WebSocketTaskProcessor(ws, task_id, streaming_mode,
                                             req['payload']['model'],
                                             req['payload']['task'], False,
                                             False, req)
                await wsc.aio_call()
        await ws.close()
    return ws
...
async def mock_sse(request):
    async with sse_response(request) as resp:
        for idx in range(10):
            ...
            print('Sending sse data: %s' % response_str)
...
async def handle_send_receive_form_data(request: aiohttp.request):
    ...
    async for field in reader:
        print('multipart field: %s' % field.name)
...
async def handle_list_file(request: aiohttp.request):
    response = {
        ...
        'data': {
            ...
            'files': [{
                ...
                11,
                'file_id':
                'da55d958-fbb2-4ed9-b979-f29af139d6f3',
                'name':
                'fine_tune_example.jsonl',
                'description':
                'testfilesfasfdsf',
                'url':
                'http://dashscope.oss-cn-beijing.aliyuncs.com/api-fs/1'
            }, {
                'id':
                10,
                'file_id':
                'fedffd0c-c247-4442-ae93-cf8525786e6c',
                'name':
                'fine_tune_example.jsonl',
                'description':
                'testfilesfasfdsf',
                'url':
                'http://dashscope.oss-cn-beijing.aliyuncs.com/api-fs/2'
            }, {
                'id':
                9,
                'file_id':
                '13ee1928-3ce4-494c-96a8-27219aec298e',
                'name':
                'fine_tune_example.jsonl',
                'description':
                'testfilesfasfdsf',
                'url':
                'http://dashscope.oss-cn-beijing.aliyuncs.com/api-fs/3'
            }]
        },
        'success': True
    }
    return web.json_response(text=json.dumps(response, ensure_ascii=True),
                             content_type='application/json')
...
async def list_models_handler(request: aiohttp.request):
    response = {
        'code': '200',
        'data': {
            'models': [
...
def create_app():
    ...
    app.router.add_post('/api/v1/services/aigc/text-generation/generation',
                        post_echo)
    app.router.add_post(
        '/api/v1/services/embeddings/text-embedding/text-embedding',
        handle_text_embedding)
    app.router.add_post('/api/v1/services/aigc/forbidden', response_403)
    app.router.add_post('/api/v1/services/aigc/image-generation/generation',
                        handle_send_receive_form_data)
    app.router.add_route('GET', '/ws/aigc/v1/echo',
                         websocket_handler_stream_none)
    app.router.add_route('GET', '/ws/aigc/v1/in', websocket_handler_stream_in)
    app.router.add_route('GET', '/ws/aigc/v1/out',
                         websocket_handler_stream_out)
    app.router.add_route('GET', '/ws/aigc/v1/inout',
    ...
    app.router.add_get('/api/v1/fine-tunes/outputs/{id}',
                       get_fine_tune_handler)
    app.router.add_delete('/api/v1/fine-tunes/{id}', delete_fine_tune_handler)
    app.router.add_delete('/api/v1/fine-tunes/outputs/{id}',
                          delete_fine_tune_handler)
    app.router.add_post('/api/v1/fine-tunes/{id}/cancel',
                        cancel_fine_tune_handler)
    app.router.add_get('/api/v1/fine-tunes/{id}/stream',
                       events_fine_tune_handler)
    ...
    app.router.add_delete('/api/v1/deployments/{id}',
                          delete_deployment_handler)
    app.router.add_get('/api/v1/deployments/{id}/events',
                       events_deployment_handler)
...
class MockServer():
    def __init__(self) -> None:
        ...
        app = web.Application()
        app.router.add_post('/api/v1/services/rerank/text-rerank/text-rerank',
                            self.handle_post)
        # fine-tune
        ...
        app.router.add_delete('/api/v1/fine-tunes/outputs/{id}',
                              self.handle_post)
        app.router.add_post('/api/v1/fine-tunes/{id}/cancel',
                            self.handle_get)  # no body
        app.router.add_get('/api/v1/fine-tunes/{id}/stream',
                           events_fine_tune_handler)
        # end of finetune
        # create assistant file
        app.router.add_post('/api/v1/assistants/{assistant_id}/files',
                            self.handle_post)
        # retrieve assistant file
        app.router.add_get('/api/v1/assistants/{assistant_id}/files/{file_id}',
                           self.handle_get)
        ...
        app.router.add_delete(
            ...
            self.handle_get)
        # list assistant file
        app.router.add_get('/api/v1/assistants/{assistant_id}/files',
                           self.handle_get)

        # create messages
        app.router.add_post('/api/v1/threads/{thread_id}/messages',
                            self.handle_create_object)
        # list messages
        app.router.add_get('/api/v1/threads/{thread_id}/messages',
                           self.handle_list_object)
        # retrieve message
        app.router.add_get('/api/v1/threads/{thread_id}/messages/{message_id}',
                           self.handle_list_object)

        # create run
        app.router.add_post('/api/v1/threads/{thread_id}/runs',
                            self.handle_create_object)
        # list runs
        app.router.add_get('/api/v1/threads/{thread_id}/runs',
                           self.handle_list_object)
        # retrieve run
        app.router.add_get('/api/v1/threads/{thread_id}/runs/{run_id}',
                           self.handle_list_object)
        # cancel run
        app.router.add_post('/api/v1/threads/{thread_id}/runs/{run_id}/cancel',
        ...
        app.router.add_post(
            ...
            self.handle_create_object)
        app.router.add_get('/api/v1/threads/{thread_id}/runs/{run_id}/steps',
                           self.handle_list_object)

        app.router.add_post('/api/v1/services/{group}/{task}/{function}',
                            self.handle_mock_request)
        app.router.add_post('/api/v1/{group}/{task}/{function}',
                            self.handle_mock_request)
        app.router.add_route('GET', '/api-ws/v1/inference',
        ...
        # delete object
        app.router.add_delete('/api/v1/{function}/{object_id}',
                              self.handle_delete_object)
        # retrieve object
        app.router.add_get('/api/v1/{function}/{object_id}',
                           self.handle_retrieve_object)
        # update with post
        app.router.add_post('/api/v1/{function}/{object_id}',
    ...
    async def handle_get(self, request: aiohttp.web.BaseRequest):
        ...
        obj = json.loads(rsp)
        return web.json_response(text=json.dumps(obj),
                                 status=status_code,
                                 content_type='application/json')
    ...
    def handle_cancel_object(self, request: aiohttp.request):
        ...
        rsp = self.responses.get(block=True)
        status_code, rsp = self.process_response(rsp)
        return web.json_response(text=rsp,
                                 status=status_code,
                                 content_type='application/json')
    ...
    def handle_retrieve_object(self, request: aiohttp.request):
        func = request.match_info['function']
        object_id = request.match_info['object_id']
        print('Retrieve %s, object_id: %s' % (func, object_id))
        self.requests.put(object_id)
        rsp = self.responses.get(block=True)
        status_code, rsp = self.process_response(rsp)
        return web.json_response(text=rsp,
    ...
    async def handle_mock_request(self, request: aiohttp.request):
        ...
        func = request.match_info['function']
        print('group: %s, task: %s, function: %s' % (group, task, func))
        body = await request.json()
...
def create_mock_server(request):
    mock_web_server = MockServer()
    proc = multiprocessing.Process(target=run_mock_server,
                                   args=(
